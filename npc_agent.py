    ...
    def sensors(self):
        ...
        sensors = [
            {'type': 'sensor.camera.rgb', 'x': 1.7, 'y': -0.4, 'z': 1.60, 'roll': 0.0, 'pitch': 0.0, 'yaw': 0.0,
             'width': 1000, 'height': 800, 'fov': 100, 'id': 'Left'},
        ]
    ...
    def run_step(self, input_data, timestamp):
        ...
        for key, val in input_data.items():
            if hasattr(val[1], 'shape'):
                # cv2.imshow('dummy',lane_detect(light_detect(val[1][:,:,:3])))
                # cv2.waitKey(50)
                # yolo = YOLO()
                
                cv2.imshow('',run(img=val[1][:,:,:3]))
                cv2.waitKey(50)
                # save_dir = f'C:/Users/22780/Documents/CARLA_0.9.13/leaderboard/scenario_runner/srunner/autoagents/video/img_{timestamp}.png'
                # status = cv2.imwrite(save_dir,val[1][:,:,:3])
                # print(status)
