...
from .block_detect import blockdetect
import sys
sys.path.append("..")
from control.pid_controller import VehiclePIDController
...
class LocalPlanner(object):
    ...
    def run_step(self,situation, debug=False):
        """
        ...
        bl = blockdetect(situation)
        object_detect = bl.object_block()
        light_detect = bl.light_detect()
        self.changelane=False
        print(object_detect)
        print(self.times)
    
        if not self.changinglane:
            if object_detect=="short distance":
                self.times += 1 
            if self.times==1000:
        ...
        if not self.changinglane:
            ...
            if object_detect == "person block" or object_detect=="short distance" or light_detect=="red light":
                control = carla.VehicleControl()
                control.steer = 0.0
                control.throttle = 0.0
                control.brake = 1.0
            ...
            elif object_detect == "middle distance":
                self.set_speed(10) #10
            elif object_detect == "long distance":
                self.set_speed(20) #20
            elif object_detect == "no engage":
                self.set_speed(50)
        ...
        max_index = -1
        # print(self._waypoints_queue[0][0].lane_change)
        # get the local planner
        for ele in self._waypoint_buffer:
            self.local_route.append(carla_location_to_numpy_vector(ele[0].transform.location))
        ...
        return control,vehicle_transform,self.local_route,self.changelane
